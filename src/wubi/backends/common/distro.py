...
'''
disk_info_re = '''(?P<name>[\w\s-]+) (?P<version>[\d.]+)(?: LTS)? \"(?P<codename>[\S\s]+)\" - (?P<subversion>[\D]+) (?P<arch>i686|amd64|x86_64) \((?P<build>[\d.]+)\)'''
...
class Distro(object):
    ...
    def is_valid_iso(self, iso_path, check_arch):
        ...
        #print "file_size === %s,size === %s,min_iso_size === %s,max_iso_size === %s" %(file_size,self.size,self.min_iso_size,self.max_iso_size)
    ...
    def get_info(self, cd_or_iso_path):
        ...
        if (cd_or_iso_path, self.info_file) in Distro.cache:
            #print cd_or_iso_path,self.info_file
            ...
            if os.path.isfile(cd_or_iso_path):
                info_file = self.backend.extract_file_from_iso(cd_or_iso_path,self.info_file,output_dir=self.backend.info.temp_dir,overwrite=True)
