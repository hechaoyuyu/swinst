...
class WindowsBackend(Backend):
    ...
    def fetch_host_info(self):
        ...
        #获取windows系统版本:xp or 2000 or 2003 or vista
    ...
    def get_uninstaller_path(self):
        '''
        uninstaller_path = []
        uninstaller_path.append(registry.get_value('HKEY_LOCAL_MACHINE', self.info.registry_key[0], 'UninstallString'))
        uninstaller_path.append(registry.get_value('HKEY_LOCAL_MACHINE', self.info.registry_key[1], 'UninstallString'))
        '''
    ...
    def get_previous_target_dir(self):
        '''
        previous_target_dir = []
        previous_target_dir.append(registry.get_value('HKEY_LOCAL_MACHINE', self.info.registry_key[0], 'InstallationDir'))
        previous_target_dir.append(registry.get_value('HKEY_LOCAL_MACHINE', self.info.registry_key[1], 'InstallationDir'))
        '''
    ...
    def get_previous_distro_name(self):
        '''
        previous_distro_name = []
        previous_distro_name.append(registry.get_value('HKEY_LOCAL_MACHINE', self.info.registry_key[0], 'DisplayName'))
        previous_distro_name.append(registry.get_value('HKEY_LOCAL_MACHINE', self.info.registry_key[1], 'DisplayName'))
        '''
    ...
    def get_registry_key(self):
        '''
        registry_key = []
        registry_key.append('Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\'  + self.info.application_name.capitalize())
        registry_key.append('Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\'  + (self.info.application_name + "_cd").capitalize())
        '''
    ...
    def get_cd_search_paths(self):
        ...
        for drive in self.info.drives:
            if drive.type == 'cd':
                listcd.append(drive.path)
                log.debug("listcd === %s" %drive)
        return listcd #[drive.path for drive in self.info.drives] # if drive.type == 'cd']
    ...
    def get_iso_file_names(self, iso_path):
        ...
        log.debug("iso_extractor === %s" %self.info.iso_extractor)
    ...
    def undo_bootini(self, drive, associated_task):
        ...
        if not os.path.isfile(bootini):
            return
        run_command(['attrib', '-R', '-S', '-H', bootini])
        remove_line_in_file(bootini, 'c:\yldr.mbr', ignore_case=True)
        remove_line_in_file(bootini, 'c:\yldrd.mbr', ignore_case=True)
        run_command(['attrib', '+R', '+S', '+H', bootini])
    ...
    def modify_configsys(self, drive, associated_task):
        ...
        if not os.path.isfile(configsys):
            return
        src = join_path(self.info.root_dir, 'winboot', 'yldr.exe')
        dest = join_path(drive.path, 'yldr.exe')
        shutil.copyfile(src,  dest)
